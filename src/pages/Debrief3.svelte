...
if (options > 0) {
  ...
  let Main_questions = [
    ...
    [
      ...
      '2. Who gets sliced in half by the red ninja early in the movie?',
      '3. Who is Hackerman and what role does he play early on?',
      '4. How does Kung Fury get his powers?',
  ...
  let Answer_a = [
    ...
    ['A) The police department','A) His sister','A) A corrupt cop who betrays Kung Fury','A) He wins a city-wide tournament','A) It is merely a background prop with no real use'],
    ['A) Kung Fury fights his former mentor','A) Only Hackerman arrives to help','A) A transportable, cellular phone','A) He betrays Kung Fury and joins the villains','A) An intelligent computer hacker']
  ];

  let Answer_b = [
    ['B) 40','B) 60','B) never at all','B) 10 ','B) Your moment by moment subjective emotional experience'],
    ['B) Barbariana','B) His father','B) A rival martial artist challenging Kung Fury','B) He kills a Viking with his bare hands and is praised by colleagues','B)  It is used to travel back in time to fight Adolf Hitler'],
  ...
  let Answer_c = [
    ...
    ['C) An electronic computer','C) His police partner','C) A hacker who uses computers to help send Kung Fury back in time','C) He defeats the entire mafia','C) It is used to transport Kung Fury to a secret island'],
  ...
  let Answer_d = [
    ...
    ['D) Thor','D) A Nazi Colonel','D) The abusive trainer of Kung Fury','D) He rescues the president','D) It is stolen by the Viking and never recovered'],
  ...
  let Answer_e = [
    ...
    ['E) Adolf Hitler','E) A rival fighter','E) A government agent tracking Nazis','E) He is struck by lightning and bitten by a cobra','E) It malfunctions and leaves Kung Fury trapped in the present'],
